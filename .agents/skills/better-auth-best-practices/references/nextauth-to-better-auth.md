...
description: A step-by-step guide to transitioning from NextAuth to Better Auth.
---

## Introduction

In this guide, we'll walk through the steps to migrate a project from [NextAuth](https://authjs.dev/) (formerly [NextAuth.js](https://next-auth.js.org/)) to Better Auth. Since these projects have different design philosophies, the migration requires careful planning and work. If your current setup is working well, there's no urgent need to migrate. We continue to handle security patches and critical issues for NextAuth.

However, if you're starting a new project or facing challenges with your current setup, we strongly recommend using Better Auth. Our roadmap includes features previously exclusive to NextAuth, and we hope this will unite the ecosystem more strongly without causing fragmentation.
...
<Tabs items={["NextAuth", "Better Auth"]}>
<Tab value="NextAuth">
...
<Callout type="info">
Now Better Auth supports stateless session management without any database. If you were using a Database adapter in NextAuth, you can refer to the [Database models](#database-models) below to check the differences with Better Auth's core schema.
</Callout>
</Step>
...
In this section, we'll look at how to manage sessions in Better Auth compared to NextAuth. For more information, see [here](/docs/concepts/session-management).

### Client-side

#### Sign In

Here are the differences between NextAuth and Better Auth for signing in users. For example, with the GitHub OAuth provider:
...
Here are the differences between NextAuth and Better Auth when signing out users.
...
Here are the differences between NextAuth and Better Auth for getting the current active session.
...
NextAuth offers approaches using Proxy (Middleware), but we recommend handling auth checks on each page or route rather than in a Proxy or Layout. Here is a basic example of protecting resources with Better Auth.
...
Both NextAuth and Better Auth provide stateless (JWT) and database session strategies. If you were using the database session strategy in NextAuth and plan to continue using it in Better Auth, you will also need to migrate your database.

Just like NextAuth has database models, Better Auth also has a core schema. In this section, we'll compare the two and explore the differences between them.
...
- `name`, `email`, and `emailVerified` are required in Better Auth, while optional in NextAuth
- `emailVerified` uses a boolean in Better Auth, while NextAuth uses a timestamp
...
<Callout type="info">
If you were using NextAuth v4, note that v5 does not introduce any breaking changes to the database schema. Optional fields like `oauth_token_secret` and `oauth_token` can be removed if you are not using them. Rarely used fields like `refresh_token_expires_in` can also be removed.
</Callout>

### Customization

You may have extended the database models or implemented additional logic in NextAuth. Better Auth allows you to customize the core schema in a type-safe way. You can also define custom logic during the lifecycle of database operations. For more details, see [Concepts - Database](/docs/concepts/database).
</Step>
</Steps>

## Wrapping Up

Now you're ready to migrate from NextAuth to Better Auth. For a complete implementation with multiple authentication methods, check out the [Next.js Demo App](https://github.com/better-auth/better-auth/tree/canary/demo/nextjs). Better Auth offers greater flexibility and more features, so be sure to explore the [documentation](/docs) to unlock its full potential.
