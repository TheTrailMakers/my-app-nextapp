...
export async function checkUserRole(requiredRole: 'SUPER_ADMIN' | 'ADMIN' | 'MODERATOR' | 'USER') {
  ...
  const user = await (prisma as any).user.findUnique({
    ...
    select: { id: true, role: true, isActive: true, isLocked: true, isDenied: true }
  });
  ...
  const roleHierarchy: { [key: string]: number } = { SUPER_ADMIN: 4, ADMIN: 3, MODERATOR: 2, USER: 1 };
  const userRoleLevel = roleHierarchy[user.role] || 0;
  const authorizedRoleLevel = roleHierarchy[requiredRole] || 0;
...
export async function trackFailedLogin(email: string, ipAddress?: string, userAgent?: string) {
  try {
    ...
    if (user) {
      await (prisma as any).failedLoginAttempt.create({
        data: {
          userId: user.id,
          email,
          ipAddress,
      ...
      const recentFailures = await (prisma as any).failedLoginAttempt.count({
        where: {
          userId: user.id,
