...
function log(message, level = "INFO") {
  ...
  const logFile = path.join(
    LOGS_DIR,
    `pipeline-${new Date().toISOString().split("T")[0]}.log`,
  );
...
async function processTrek(trekData, progress) {
  ...
  try {
    ...
    const research = aggregateResearchData(
      trekData,
      wikiArticle,
      webData || { ddgData: null, wikiSummary: null, sources: [] },
    );

    // Log data quality
    const hasWikiData = research.wikiExtract.length > 100;
    const hasWebData =
      research.ddgAbstract.length > 50 ||
      research.wikiSummaryExtract.length > 100;
    log(
      `  📊 Data quality: Wikipedia=${hasWikiData ? "✓" : "✗"}, Web=${hasWebData ? "✓" : "✗"}`,
    );

    if (!hasWikiData && !hasWebData) {
      log(
        `  ⚠️  Insufficient data for ${name}. Flagging for manual review.`,
        "WARN",
      );
    ...
    try {
      ...
      content.tagline = "";
      content.aboutText =
        trekData.longDescription || trekData.description || "";
    }
    ...
    try {
      ...
      content.difficultyDetails = {
        overall: trekData.difficulty,
        justification: "",
      };
    }
    await sleep(LLM_CALL_DELAY);
    ...
    try {
      ...
      content.permits = {
        ...
        summary: "Check with local authorities",
      };
    }
    await sleep(LLM_CALL_DELAY);
    ...
    try {
      ...
      content.gearChecklist = gearFaqs.gearChecklist || {
        essential: [],
        clothing: [],
        doNotCarry: [],
      };
      ...
      log(
        `    ✓ Gear (${content.gearChecklist.essential?.length || 0} items) & FAQs (${content.faqsJson.length}) generated`,
      );
    } catch (err) {
    ...
    try {
      ...
      content.seoDescription =
        seoData.seoDescription || trekData.description?.slice(0, 155) || "";
      content.seoKeywords = [
    ...
    if (errors.length > 0) {
      log(
        `  ⚠️  ${errors.length} section(s) had errors: ${errors.join(", ")}`,
        "WARN",
      );
    }
...
async function main() {
  ...
  if (RESUME && progress.completed.length > 0) {
    console.log(
      `📂 Resuming from progress: ${progress.completed.length} already completed\n`,
    );
  }
      ...
      if (result.errors?.length > 3) {
        ...
        if (!progress.flagged.find((f) => f.slug === trek.slug)) {
          progress.flagged.push({
            slug: trek.slug,
            reason: `${result.errors.length} section errors`,
          });
        }
      }
