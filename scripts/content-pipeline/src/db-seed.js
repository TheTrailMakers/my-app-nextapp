...
async function main() {
  ...
  console.log(`  Verify content: ${VERIFY ? 'YES' : 'NO (contentVerified=false)'}`);
  ...
  for (const file of files) {
    ...
    try {
      ...
      const existingTrek = await prisma.trek.findUnique({ where: { slug } });
      ...
      const updateData = {
        ...
        itineraryJson: trekContent.itineraryJson ? JSON.stringify(trekContent.itineraryJson) : null,
        difficultyDetails: trekContent.difficultyDetails ? JSON.stringify(trekContent.difficultyDetails) : null,
        gearChecklist: trekContent.gearChecklist ? JSON.stringify(trekContent.gearChecklist) : null,
        faqsJson: trekContent.faqsJson ? JSON.stringify(trekContent.faqsJson) : null,
        bestTimeJson: trekContent.bestTimeJson ? JSON.stringify(trekContent.bestTimeJson) : null,
        gettingThere: trekContent.gettingThere ? JSON.stringify(trekContent.gettingThere) : null,
        permits: trekContent.permits ? JSON.stringify(trekContent.permits) : null,
        ...
        schemaMarkup: trekContent.seo?.schemaMarkup ? JSON.stringify(trekContent.seo.schemaMarkup) : null,
        
        // Content management
        contentVerified: VERIFY ? true : false,
        contentGeneratedAt: trekContent.contentGeneratedAt ? new Date(trekContent.contentGeneratedAt) : new Date(),
        contentSources: trekContent.sourcesUsed || [],
      };

      // Also update inclusions/exclusions if they were improved
      if (trekContent.inclusions && trekContent.inclusions.length > 0) {
        updateData.inclusions = trekContent.inclusions;
      }
      if (trekContent.exclusions && trekContent.exclusions.length > 0) {
        updateData.exclusions = trekContent.exclusions;
      }

      if (DRY_RUN) {
        console.log(`  🔍 DRY RUN: Would update ${slug}`);
        console.log(`     - tagline: ${updateData.tagline?.slice(0, 60) || 'null'}`);
        console.log(`     - aboutText: ${updateData.aboutText?.slice(0, 80) || 'null'}...`);
      ...
      if (trekContent.flaggedForReview) {
        console.log(`  ⚠️  Flagged for review (generation errors or conflicts)`);
      }
      stats.updated++;
